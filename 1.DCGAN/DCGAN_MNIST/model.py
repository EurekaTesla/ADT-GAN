...
def G_model(Height, Width, channel):
    ...
    x = Conv2DTranspose(256, (4, 4), name='g_conv1', padding='valid', strides=(1,1),
    ...
    x = Conv2DTranspose(128, (3, 3), name='g_conv2', padding='same', strides=(2,2),
        kernel_initializer=RN(mean=0.0, stddev=0.02), use_bias=False)(x)
    x = Activation('relu')(x)
    x = BatchNormalization(momentum=0.9, epsilon=1e-5, name='g_conv2_bn')(x)
   
    x = Conv2DTranspose(channel, (3, 3), name='g_out', padding='same', strides=(2,2),
...
def D_model(Height, Width, channel):
    ...
    x = Conv2D(128, (3, 3), padding='same', strides=(2,2), name='d_conv1',
        kernel_initializer=RN(mean=0.0, stddev=0.02), use_bias=False)(inputs)
    x = LeakyReLU(alpha=0.2)(x)

    x = Conv2D(256, (3, 3), padding='same', strides=(2,2), name='d_conv2',
        kernel_initializer=RN(mean=0.0, stddev=0.02), use_bias=False)(x)
    x = LeakyReLU(alpha=0.2)(x)

    x = Conv2D(512, (3, 3), padding='same', strides=(2,2), name='d_conv3',
        kernel_initializer=RN(mean=0.0, stddev=0.02), use_bias=False)(x)
    x = LeakyReLU(alpha=0.2)(x)
